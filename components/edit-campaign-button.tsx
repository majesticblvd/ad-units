...
export function EditCampaignButton({
	...
	const handleSave = async () => {
		...
		const didMoveCampaign =
			selectedCampaignId && selectedCampaignId !== currentCampaignId;
		const didChangeTitle = (currentTitle || null) !== nextTitle;
		const didChangeDescription =
			(currentDescription || null) !== nextDescription;
		...
		try {
			...
			if (didMoveCampaign) {
				...
				const { data: highestPositionData, error: positionError } =
					await supabase
						.from("ads")
						.select("position")
